...
void walkRegions(ArrayRef<Instr> instrs, const FunctionBoundaries &boundaries,
    ...
    for (size_t f = 0; f < fn_list.size() - 1; ++f) {
        ...
        // Fast O(N log N) abab detection with 10-chain threshold + contiguous basic block gap merging
        ...
        if (n_loops > 0) {
            ...
            // Fast sweep-line to mark abab loops and compute chain depth
            ...
            int threshold = opts::ChainThreshold;
        ...
        while (g_idx < f_size) {
            ...
            auto it = std::lower_bound(loops.begin(), loops.end(), g_end, [](const SimpleLoop &l, size_t val) {
                return l.h_idx < val;
            });
            if (it != loops.end()) {
                following_loop = &(*it);
